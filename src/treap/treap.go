...
// Find ClosestKey key greater than or equal to Key;
// that is, smallest item not less than the key.
// Only returns nil if all items are smaller than the key.
func (t *Tree) ClosestKey(key Key) Item {
	return t.closestKey(t.root, key)
}

func (t *Tree) closestKey(node *Node, key Key) Key {
	if node == nil {
		return nil
	}
	if t.less(key, node.key) {
		if node.left == nil {
			// This is the smallest node not less than key.
			return node.key
		}
		return t.closestKey(node.left, key)
	}
	if t.less(node.key, key) {
		return t.closestKey(node.right, key)
	}
	return node.key
}
...
func (t *Tree) IterAscendStartingAt(key Key) <-chan Item {
	c := make(chan Item)
	go func() {
		iterateInOrderStartingAt(key, t.less, t.root, c)
		close(c)
	}()
	return c
}

func iterateInOrderStartingAt(key Key, less LessFunc, h *Node, c chan<- Item) {
	if h == nil {
		return
	}
	if less(key, h.key) {
		iterateInOrderStartingAt(key, less, h.left, c)
		c <- h.item
	}
	iterateInOrderStartingAt(key, less, h.right, c)
}
